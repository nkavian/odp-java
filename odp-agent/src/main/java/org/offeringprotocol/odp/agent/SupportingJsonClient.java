...
final class SupportingJsonClient {
    ...
    OdpJsonNode get(URI target, String accept, Set<String> mediaTypes, int maximumBytes, int maximumDepth) {
        ...
        for (int redirects = 0; redirects <= MAXIMUM_REDIRECTS; redirects++) {
            HttpRequest request = HttpRequest.newBuilder(current)
                    ...
                    .GET()
                    .build();
            HttpResponse<byte[]> response = send(request, maximumBytes, retry);
            ...
            if (isRedirect(status)) {
                ...
                if (bytes.length > maximumBytes) {
                    throw new OdpResponseLimitException("ODP supporting resource exceeds its byte limit");
                }
                String contentType =
                ...
                if (depth(document, maximumDepth) > maximumDepth) {
                    throw new OdpResponseLimitException("ODP supporting resource exceeds its JSON depth limit");
                }
                return document;
            }
        }
    ...
    private HttpResponse<byte[]> send(HttpRequest request, int maximumBytes, ReadRetry retry) {
        try {
            return retry.send(transport, request, maximumBytes);
