...
final class Catalog {
    ...
    static OdpService multilingual(String... localizations) {
        CatalogHandler handler = request -> {
            var value = org.offeringprotocol.odp.core.OdpJson.valueToTree(
                    offering("plant-1", "fr".equals(request.language()) ? "Plante" : "Plant"));
            value.put("language", request.language());
            if (request.identifier() != null) {
                return value;
            }
            value.remove("odp_version");
            return new Page<>(null, Odp.VERSION, List.of(value), null, Map.of());
        };
        ...
        return new OdpService(
                ...
                Map.of(OdpOperation.LIST_OFFERINGS, endpoint, OdpOperation.GET_OFFERING, endpoint));
    }
