...
class LanguageConformanceTest {
    ...
    @Test
    void doesNotRelabelFixedMetadataOrStaticCatalogs() {
        OdpService fixed = new OdpService(
                Catalog.template(List.of("en", "fr")),
                StaticCatalog.create(List.of(Catalog.offering("plant-1", "Rubber Plant")), List.of()));
        for (String path : List.of(Odp.SERVICE_DOCUMENT_PATH, OFFERINGS, OFFERINGS + "/plant-1")) {
            OdpHttpResponse english = fixed.handle(with(path, "Accept-Language", "en"));
            OdpHttpResponse french = fixed.handle(with(path, "Accept-Language", "fr"));
            assertEquals(200, french.status());
            assertEquals("en", french.headers().get("Content-Language"));
            assertEquals(english.body(), french.body());
            assertEquals(english.headers().get("ETag"), french.headers().get("ETag"));
        }
    }

    @Test
    void doesNotUseUnknownPageFieldsAsLanguageHeaders() {
        OdpService empty = Catalog.handling(
                request -> Map.of("odp_version", "1.0", "items", List.of(), "language", "untrusted\r\nheader"));
        OdpHttpResponse response = empty.handle(get(OFFERINGS));
    ...
    @Test
    void triesSpecificRangesBeforeWildcardDefault() {
        assertEquals(
                "fr",
                service.handle(with(OFFERINGS, "Accept-Language", "*, fr;q=0.9"))
                        .headers()
                        .get("Content-Language"));
    }

    @Test
    void ignoresInvalidQualityWeights() {
        for (String quality : List.of("NaN", "Infinity", "1.5", "-0.5", "nonsense")) {
            assertEquals(
                    "en",
                    service.handle(with(OFFERINGS, "Accept-Language", "fr;q=" + quality + ", en;q=0.5"))
                            .headers()
                            .get("Content-Language"),
                    quality);
        }
